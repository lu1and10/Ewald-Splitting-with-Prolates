...
#SBATCH --output=strong_pme_p5_%A_%a.out
...
RUNDIR=$SLURM_SUBMIT_DIR/strong-pme-p5-$SLURM_NNODES-$SLURM_NTASKS-$SLURM_ARRAY_TASK_ID
mkdir -p $RUNDIR

#cp $SRCDIR/nvt-pme-9cut-p5-1.5e-5-64m-single.tpr $RUNDIR
cp $SRCDIR/nvt-pme-9cut-p5-1.5e-5-64m.mdp $RUNDIR
cd $RUNDIR
...
mpiexec --map-by socket:pe=1 $SRCDIR/gmx_mpi mdrun -s $SRCDIR/nvt-pme-9cut-p5-1.5e-5-64m-single.tpr -dlb no -deffnm nvt-pme-9cut-p5-1.5e-5-64m-single -npme 0 -notunepme -cpt -1
